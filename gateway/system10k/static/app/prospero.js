...
if(!localStorage.sessionId || !localStorage.email || !localStorage.role || !localStorage.teammate) {
...
function loadPage() {
    document.title = teamId.slice(0, 4) + ' | Prospero.Live'
...
function handlePeerMessage(message, replyChannel, toEmail) {
    ...
    if(message.type == 'editor-changes') {
        ...
        if(editor) {
            ...
            collabDebugLog.push(JSON.parse(JSON.stringify(match)))
            ...
            shiftChange(otherChanges, change, checksumTable, match)
