function Algorithm() { 
    ...
    function inputHandler(event, node) {
        ...
        console.log('input', node.state.peerRevisionId)
        ...
        var change = {
            ...
            peerRevisionId: node.state.peerRevisionId,
            // origin: event.origin,
        }
        ...
        var message = {
            ...
            // maxReceivedRevisionId: node.state.maxReceivedRevisionId,
        }
    ...
    function clientChangeHandler(event, node) {
        ...
        node.state.peerRevisionId = message.revisionId
        ...
        // else if(otherChanges.length > 0) {
            ...
            // otherChanges.forEach(function(otherChange) {
            //     if(otherChange.madeBy != message.change.madeBy) {
            //         var document
            //         var checksum

            //         document = 

            //         checksum = crc32(document.toString('binary'))
        ...
        clientShiftChange(otherChanges, change, node, match)
    ...
    function applyChange(message, node, isRealMessage, event) {
        ...
        var change = message.change
        event.preTransformedChange = deepCopy(change)
        serverShiftChange(otherChanges, change, node, match)
        event.transformedChange = deepCopy(change)
    ...
    clientShiftChange = shiftChange
    serverShiftChange = shiftChange

    function shiftChange(missedChanges, proposedChange, node, match) {
        // var extraPatches = []
        var newChanges = []

        // Transforms partner history through your history ->
        missedChanges.forEach(function(missedChange, i) {
            console.log(missedChange.revisionId, proposedChange.peerRevisionId)
            if(missedChange.madeBy != proposedChange.madeBy //) {
