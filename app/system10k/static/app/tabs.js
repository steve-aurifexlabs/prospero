...
function updateTabCount() {
    var tabs = document.querySelector('#tabs')
    innerUpdateTabCount(tabs, true, 245)

    var otherTabs = document.querySelector('#other-tabs')
    innerUpdateTabCount(otherTabs, false, 200)
}

function innerUpdateTabCount(tabs, extraSpace, startingWidth) {
    var w = document.querySelector('#doc-header').clientWidth
    // var tab = tabs.querySelector('div')


    var extraSpaceWidth = 10
    if(extraSpace) {
        extraSpaceWidth = 22.2
    }
    ...
    if(tabs.children.length * startingWidth > w) {
        ...
        tabs.childNodes.forEach(function(tab) {
            ...
            tab.childNodes[0].style.width = (w / tabs.children.length) - extraSpaceWidth + 'px'
        })
...
function switchTab(tab) {
    ...
    tab.style.borderColor = document.querySelector('#your-element').style.borderColor
    ...
    if(runButton) {
        runButton.addEventListener('click', function() {
            ...
            fetch('/api/run/start-run-session?path=' + encodeURIComponent(path), {
                ...
                var hostname = 'https://' + data.runSessionUrlPrefix + '.' + envPrefix + 'run.' + location.host.split('.').slice(-3, -2) + '.prospero.live'
                ...
                p5.innerHTML = "Please inspect the following command line invocations and only run them if you understand what they do. You should run these commands as a regular user, i.e., <b>no sudo</b>."
    ...
    if(runButton2) {
        runButton2.addEventListener('click', function() {
            ...
            fetch('/api/run/start-run-session?path=' + encodeURIComponent(path), {
                ...
                window.open(
                    'https://' + data.runSessionUrlPrefix + '.' + envPrefix + 'run.prosperodev.live/run' + tab.key.slice(5) + '?otp=' + data.runSessionOtp,
                    '_blank',
                )
