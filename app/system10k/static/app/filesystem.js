...
function promptForPayment() {
    ...
    if(storageUsed === undefined || storageLimit === undefined) {
        h2.textContent = "The leader of this team is over their storage limit. Create a new team to use your storage or suggest they upgrade. It's only $3 to start."
        a.textContent = 'Click here to create a new team, switch teams, or upgrade your account.'
        a.setAttribute('href', 'https://prospero.live/user/dashboard')        
    }

    else {
        h2.textContent = 'You are over your storage limit.'

        if(storageLimit < 50 * 1024 * 1024) {
            a.textContent = 'Click here to upgrade your account for only $3.'
            a.setAttribute('href', 'https://prospero.live/user/select-plan')        
        }

        else {
            a.textContent = 'Click here to upgrade your account to access more storage.'
            a.setAttribute('href', 'https://prospero.live/user/dashboard')        
        }
    }
...
function downloadWorkspace(path, name) {
	 var progressElement = document.querySelector('progress')
        if(!progressElement) {
            progressElement = document.createElement('progress')
            progressElement.style.position = 'fixed'
	...
	fetch('/api/fs/downloadFolder?path=' + encodeURIComponent(path) + '&filename=' + encodeURIComponent(name) + '&team=undefined', {
            method: 'GET',
            headers: {
                Authorization: 'Bearer ' + localStorage.sessionId,
            },
        ...
        .then(function(blob) {
            ...
            if(path) {
                ...
                a.download = 'prospero-' + teamId.slice(0, 4) + '.zip'
            }
            document.body.appendChild(a)
            a.click()    
            a.remove()
